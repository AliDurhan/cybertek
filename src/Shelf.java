...
// BU ASAGIDA UZUNCA IFADE ETTIGIMIN KISA VERSIYONU
    	// .isEmpty() metoda zaten bize TRUE ya da FALSE return ediyor
    	// onun icin uzun birseye gerek yok direk alttaki ifade yeterli
...
//    		return false;
    ...
    public static boolean isIDUnique(int id) {
    	...
    	for (int i = 0; i < theBooks.size(); i++) {
    		...
    		// one-line statement oldugu icin paranteze gerek yok!
    ...
    public static String getTitleByID(int id) {
        ...
        for (int i = 0; i < theBooks.size(); i++) {
        	...
        	if (theBooks.get(i).getId() == id ) return theBooks.get(i).getTitle();
...
//  NE KADAR SACMA YAPMISIM ASAGIDA YAWS!!!!
    	
        for (int i = 0; i < theBooks.size(); i++) {
        	
        if (theBooks.get(i).getTitle().toLowerCase().contains(title.toLowerCase())) return theBooks.get(i);
        	
		}
        
// BU DA UZUN YOLDAN YAPIMI YUKARIDA KISALTTIM
        	
//        title = title.toLowerCase();
//        
//        	String objTitle = theBooks.get(i).getTitle().toLowerCase();
//        	
//        	if (objTitle.contains(title) ) {
    ...
    public static List<Book> getAllBooksForAuthor(String author) {
    	...
    	for (int i = 0; i < theBooks.size(); i++) {
			if (theBooks.get(i).getAuthor().equalsIgnoreCase(author)) {
				booksByAuthor.add(theBooks.get(i));
			}
